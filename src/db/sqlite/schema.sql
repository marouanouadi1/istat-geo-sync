...
CREATE TABLE
  IF NOT EXISTS provinces (
    ...
    nuts3_2021 TEXT,
    nuts3_2024 TEXT,
    FOREIGN KEY (region_code) REFERENCES regions (istat_region_code) ON UPDATE CASCADE ON DELETE RESTRICT
  );
...
CREATE TABLE
  IF NOT EXISTS municipalities (
    ...
    nuts3_2024 TEXT,
    FOREIGN KEY (region_code) REFERENCES regions (istat_region_code) ON UPDATE CASCADE ON DELETE RESTRICT,
    FOREIGN KEY (province_uts_code) REFERENCES provinces (uts_code) ON UPDATE CASCADE ON DELETE RESTRICT
  );
...
CREATE TABLE
  IF NOT EXISTS notes (note_id TEXT PRIMARY KEY, text TEXT NOT NULL);
